...
- **手动推送** — 设置页一键推送今日课表、7天总览、今日学习任务、今日/本周总结，方便测试和修改后补发
...
学习任务推送帮助家长把孩子的每日任务和完成情况自动同步到钉钉群：

**定时推送：**

- **今日学习任务（每天 07:00）** — 列出今天需要完成的任务（含任务名、科目、emoji、预计时长），复用今日任务筛选口径
- **今日完成总结（每天 21:00）** — 当天完成的任务、完成率、获得积分，统计口径与今日任务一致
- **本周总结（每周日 22:00）** — 本周完成次数、完成率、打卡天数、积分、分科目分布，复用周报统计口径
- 开机自动补发：若当天尚未推送今日任务，服务启动时立即补推一次

**推送机制：**

- 复用课程推送的钉钉 webhook 配置与开关，无需单独配置
- 全部推送经熔断保护（`runSafely + pushBreaker`），避免数据库/网络故障时重复刷屏
- 手动推送：设置页「学习任务推送」卡片可一键触发今日任务、今日总结、本周总结
- 消息均含钉钉安全关键词「励夏的课程」

**手动推送端点：**

- `POST /api/schedule-push-config/task/today` — 今日学习任务
- `POST /api/schedule-push-config/task/daily-summary` — 今日完成总结
- `POST /api/schedule-push-config/task/weekly-summary` — 本周总结
...
📝 **v0.4.0** — 课程表钉钉推送 + 学习任务推送 + 7天总览 + 上课提醒
